...
public class dischargeLetterController {
    ...
    public void initData(Ricovero ric, LoggedUser userAccount){
        ...
        primario = (DataBase.getDataBaseInstance()).getMedicoFromLoggedUser(userAccount);
        if(primario != null) {
    ...
    public void confirm(){
        ...
        if(dataFineRicFXML.getText().matches("\\d{2}([\\-]\\d{2})([\\-]\\d{4})?")) {
        ...
        }else{showAlert(Alert.AlertType.ERROR, contPane.getScene().getWindow(), "Errore", "Inserisci una data corretta!");}
    }
