...
public class DataBase{
    ...
    public LoggedUser login(String username, String password, String tableName, String dbName) throws SQLException {
        String pathDB = new String("jdbc:mariadb://localhost:3306/" + dbName + "?user=userGen&pwd=");
        ...
        try {
            ...
            resultSet = statement.executeQuery("SELECT userName, password, tipoAccount, CodFiscale FROM " + dbName + "." + tableName + " WHERE userName='" + username + "' AND password='" + password + "';");
            resultSet.next();
            if(resultSet.getString("userName") != "" && resultSet.getString("password") != ""){
                ...
                resultSet = statement.executeQuery("Select cod_fiscale FROM " + dbName + "." + tableName + " WHERE " + tableName + ".cod_fiscale = '" + userAccount.getCodFiscale() + "'");
                resultSet.next();
    ...
    //Ritorna la chiave per effettuare il login sul db principale
    public String[] getAccountKeyForLogin(String usrName, String pwd) throws SQLException{
        connection = DriverManager.getConnection("jdbc:mariadb://localhost:3306/accountterapiaintensiva?user=genUser");
        statement = connection.createStatement();
        System.out.println("select accounttype from accountterapiaintensiva.account where username = \"" + usrName + "\" AND password = \"" + pwd + "\"");
        resultSet = statement.executeQuery("select accounttype from accountterapiaintensiva.account where username = \"" + usrName + "\" AND password = \"" + pwd + "\"");
        resultSet.next();
        String accType = new String(resultSet.getString("accounttype"));
        System.out.println("select keyForLogin, description from accounttype where codAccount = " + accType);
        resultSet = statement.executeQuery("select keyForLogin, description from accounttype where codAccount = " + accType);
        resultSet.next();
        System.out.println("KEY: " + resultSet.getString("keyForLogin"));
        String[] returnValue = {resultSet.getString("description"), resultSet.getString("keyForLogin")};
        return returnValue;
    }
    ...
    public boolean insertPatient(String nome, String cognome, Date dataNascita, String luogoNascita, String codSanitario, Label errorLabel){
        errorLabel.setText("");
        this.connectToDb();
        ...
        try {
            ...
            if(resultSet.next()){
                errorLabel.setText("Paziente già presente nel sistema!!!");
                return false;
            }
        }catch (SQLException e){
            errorLabel.setText("Errore generico SQLException!");
            close();
            return false;
        }
        ...
        if(codSanitario.length() != 20){
            errorLabel.setText("Codice sanitario inserito non corrisponde a quello richiesto (lunghezza 20)!");
            close();
            return false;
        }
    ...
    public void setUserAccount(LoggedUser userAccount) {
    ...
    public boolean dischargePatient(Ricovero ricovero){
        String fileName = "E:\\Users\\david\\IdeaProjects\\LettereDimissioni\\" + ricovero.getIdRicovero() + "_" + ricovero.getDataInizioRicovero().getTime()+".pdf";
        return false;
    ...
    public ArrayList<Paziente> searchPatient(String nome, String cognome, String codSanitario){
    ...
    public ArrayList<Paziente> loadPatientList() {
        ArrayList<Paziente> listaPazienti = new ArrayList<>();
        connectToDb();
    ...
    public ArrayList<Farmaco> searchFarmaco(String denominazione, String codAIC, String principioAttivo){
        int codAICInt;
        ArrayList<Farmaco> farmaci = new ArrayList<>();

        if(codAIC != null && codAIC != "")
            codAICInt = Integer.getInteger(codAIC);
